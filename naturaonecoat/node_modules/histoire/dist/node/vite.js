...
import { resolveConfig as resolveViteConfigInternal, mergeConfig as mergeViteConfig, loadConfigFromFile as loadViteConfigFromFile, } from 'vite';
...
export async function resolveViteConfig(ctx) {
    const command = ctx.mode === 'dev' ? 'serve' : 'build';
    let viteConfig = (await resolveViteConfigInternal({}, command));
    viteConfig = mergeHistoireViteConfig(viteConfig, ctx);
    return viteConfig;
}
async function mergeHistoireViteConfig(viteConfig, ctx) {
    if (ctx.config.vite) {
        const command = ctx.mode === 'dev' ? 'serve' : 'build';
